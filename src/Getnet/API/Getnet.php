...
class Getnet
{
     ...
     * @return BaseResponse
     */
    public function cofre(Transaction $transaction)
    {
        try {
            $request = new Request($this);
            $response = $request->post($this, "/v1/cards", $transaction->toJSON());
        } catch (\Exception $e) {
        ...
        $cofreresponse = new BaseResponse();
        $cofreresponse->mapperJson($response);
        return $cofreresponse;
    }

    /**
     * Lista de cartões salvos no cofre pelo customer_id
     * 
     * @param $customer_id
     * @return BaseResponse
     */
    public function getCofreByCustomerId($customer_id)
    {

        try {
            $request = new Request($this);
            $response = $request->get($this, "/v1/cards?customer_id=" . $customer_id);
        } catch (\Exception $e) {
    ...
    /**
     * Recupera um cartão salvo no cofre
     * 
     * @param $card_id
     * @return BaseResponse
     */
    public function getCardByCardId($card_id)
    {
        ...
        try {
            $request = new Request($this);
            $response = $request->get($this, "/v1/cards/" . $card_id);
        } catch (\Exception $e) {
    ...
    /**
     * Remove um cartão salvo no cofre
     * 
     * @param $card_id
     * @return BaseResponse
     */
    public function removeCardByCardId($card_id)
    {

        try {
            $request = new Request($this);
            $response = $request->delete($this, "/v1/cards/" . $card_id);
        } catch (\Exception $e) {
